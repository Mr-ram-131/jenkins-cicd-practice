    ...
    steps {
        withCredentials([file(credentialsId: 'newec2-key-file', variable: 'SSH_KEY')]) {
            bat '''
                ...
                ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no ubuntu@13.235.67.74 "cd ~/git_practice && git pull origin master && docker build -t cicd-flask-app:latest . && docker stop cicd-flask-app || true && docker rm cicd-flask-app || true && docker run -d --name cicd-flask-app -p 5000:5000 cicd-flask-app:latest && sleep 5 && curl -f http://localhost:5000/health"
            '''
        }
    }
}
